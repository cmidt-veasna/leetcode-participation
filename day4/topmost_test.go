package day4

import (
	"testing"

	"github.com/stretchr/testify/require"
)

var topmostCase = [][][]int{
	{{2}, {1, -1}},
	{{5, 2, 2, 4, 0, 6}, {4, 5}},
	{{99, 95, 68, 24, 18}, {69, 99}},
	{{17, 61, 5, 1, 44}, {100, 61}},
	{{1, 2, 1000000000}, {2, 1000000000}},
	{
		{35, 43, 23, 86, 23, 45, 84, 2, 18, 83, 79, 28, 54, 81, 12, 94, 14, 0, 0, 29, 94, 12, 13, 1, 48, 85, 22, 95, 24, 5, 73, 10, 96, 97, 72, 41, 52, 1, 91, 3, 20, 22, 41, 98, 70, 20, 52, 48, 91, 84, 16, 30, 27, 35, 69, 33, 67, 18, 4, 53, 86, 78, 26, 83, 13, 96, 29, 15, 34, 80, 16, 49},
		{15, 94},
	},
	{
		{91, 98, 17, 79, 15, 55, 47, 86, 4, 5, 17, 79, 68, 60, 60, 31, 72, 85, 25, 77, 8, 78, 40, 96, 76, 69, 95, 2, 42, 87, 48, 72, 45, 25, 40, 60, 21, 91, 32, 79, 2, 87, 80, 97, 82, 94, 69, 43, 18, 19, 21, 36, 44, 81, 99},
		{2, 91},
	},
	{
		{1, 2, 3, 4, 1}, {1000000000, 4},
	},
	{{0, 1, 2}, {3, 1}},
	{{100, 9, 6, 8, 7}, {5, 100}},
}

func init() {
	aa := make([][]int, 2)
	aa[0] = make([]int, 100000)
	aa[1] = []int{1000000000, 99999}
	for i := 0; i < 100000; i++ {
		aa[0][i] = i
	}
	topmostCase = append(topmostCase, aa)
}

func TestMaximumTops(t *testing.T) {
	for i, tc := range topmostCase {
		t.Logf("TestMaximumTops case #%d", i+1)
		result := maximumTop(tc[0], tc[1][0])
		require.Equal(t, tc[1][1], result)
	}
}
